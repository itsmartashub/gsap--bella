...
!(function (e, t) {
	...
	var u,
		...
		D = function _getDocScrollTop() {
			return (
				d.pageYOffset || u.scrollTop || i.scrollTop || r.scrollTop || 0
			);
		},
		...
		P = function _placeSiblings(e, t) {
			...
			if (
				...
				(i = l ? { x: 0, y: 0 } : e.getBBox()),
					(a = (n = e.transform ? e.transform.baseVal : {})
						.numberOfItems
						? ((r =
								(n =
									1 < n.numberOfItems
	...
	function getGlobalMatrix(e, t, o) {
		if (!e || !e.parentNode || (u || b(e)).documentElement === e)
			return new he();
		...
		if ((c.removeChild(r), n))
			for (a = n.length; a--; )
				((s = n[a]).scaleX = s.scaleY = 0), s.renderTransform(1, s);
		...
		function ai(e) {
			...
			if (C) {
				...
				q
					...
					: G
					? (U &&
							((Z.deltaY = u - parseFloat(se.y)),
							(se.y = u + 'px')),
					  j &&
							((Z.deltaX = p - parseFloat(se.x)),
							(se.x = p + 'px')),
					  se.renderTransform(1, se))
					: (U &&
							((Z.deltaY = u - parseFloat(g.style.top || 0)),
							(g.style.top = u + 'px')),
					  j &&
							((Z.deltaY = p - parseFloat(g.style.left || 0)),
							(g.style.left = p + 'px'))),
					!f ||
						e ||
						A ||
		...
		function bi(e, t) {
			...
			g._gsap || (se = fe.core.getCache(g)),
				G
					...
					: ((Z.y =
							parseInt(
								g.style.top || ((n = Ia(g)) && n.top),
								10
							) || 0),
					  (Z.x =
							parseInt(g.style.left || (n || {}).left, 10) || 0)),
				(H || z || I) &&
					!t &&
					...
					: p.bounds &&
					  ...
					  q
							...
							? ((t = Ma(g, g.parentNode)),
							  (Z.minX = L =
									Math.round(
										uh(d, 'px') + e.left - t.left - 0.5
									)),
							  (Z.minY = E =
									Math.round(
										uh(K, 'px') + e.top - t.top - 0.5
									)),
							  (Z.maxX = D =
				...
				h
					...
					: (de() ? (bi(!0, !0), ei()) : Z.applyBounds(),
					  q
							? ((e = g.ownerSVGElement
									? [
											se.xOrigin - g.getBBox().x,
											se.yOrigin - g.getBBox().y,
									  ]
									: (Ia(g)[Ee] || '0 0').split(' ')),
							  (P = Z.rotationOrigin =
		...
		function mi(e, t) {
			...
			if (
				...
				if (
					...
					(Se && y) ||
						(ua(ce, 'mouseup', ge),
						e && e.target && ua(e.target, 'mouseup', ge)),
				...
				if (
					...
					(F = !N && !Z.allowEventDefault) &&
						(wa(e), ua(me, 'touchforcechange', wa)),
					e.changedTouches
				...
				if (u && !ke && Z.isPressed && e) {
					...
					(((r = Math.abs(o - b)) !== (a = Math.abs(n - w)) &&
						...
						: ((F =
						  ...
						  Z.autoScroll && (ee = !0),
						  ue(
								e.pageX - (le && q ? Ba(ce) : 0),
								e.pageY - (le && q ? Aa(ce) : 0),
								l
						  ));
				} else F && e && u && wa(e);
			},
					...
					if ((sa(ai), !q))
						for (n = J.length; -1 < --n; )
							Ha(
								J[n],
								'cursor',
								p.cursor || (!1 !== p.cursor ? Le : null)
							);
					if (
						(l && (($ = Ye = He()), (Z.isDragging = !1)), Te--, e)
					) {
						if (
							(o = e.changedTouches) &&
							(e = o[0]) !== m &&
		...
		return (
			...
			(e.update = function (e, t, o) {
				...
				return (
					...
					Z.isPressed &&
						!t &&
						((j && 0.01 < Math.abs(n - Z.x)) ||
							(U && 0.01 < Math.abs(i - Z.y) && !q)) &&
						ji(),
					Z.autoScroll &&
						(Ga(g.parentNode, Z.isDragging),
						(ee = Z.isDragging),
						ai(!0),
						Da(g, qi),
						Ca(g, qi)),
			...
			(e.enable = function (e) {
				...
				if (
					...
					fe.utils.checkPrefix('touchCallout') &&
						(i.touchCallout = 'none'),
					(i.touchAction =
						j == U
							? 'none'
	...
	function yb(e, t, o) {
		var n = j.createElementNS
			? j.createElementNS('svg' === e ? se : le, e)
			: j.createElement(e);
		return (
			t && (ob(t) && (t = j.querySelector(t)), t.appendChild(n)),
			'svg' === e &&
				(n.setAttribute('xmlns', se),
				n.setAttribute('xmlns:xlink', le)),
			o && (n.style.cssText = o),
			n
		);
	}
	...
	var q,
		...
		ge = function GSDevTools(r) {
			...
			function Eo(e) {
				...
				t &&
					((t._inProgress = zn('in') || 0),
					(t._outProgress = zn('out') || 100)),
					r.paused && A(),
					(i = null),
					to(a || t || Z);
			...
			var u,
				...
				n = (function _createRootElement(e, t, o) {
					...
					var n = yb(
						'div',
						e || Q.getElementsByTagName('body')[0] || Q
					);
					return (
						n.setAttribute(
							'class',
	...
	(ge.version = '3.3.4'),
		...
		(ge.getByAnimation = function (e) {
			for (var t in (ob(e) && (e = q.getById(e)), de))
